...
class ManualAdjustor:
    ...
    def named_team_adjustment(self, df):
        team_list = self.format_proper_team_names(MANUAL_TEAM_ADJUSTMENT)
        df['elo_team'] = df.apply(lambda row: row['elo_team'] + MANUAL_TEAM_ELO_ADJUSTMENT if row['team'] in team_list else row['elo_team'], axis=1)
        df['elo_opponent'] = df.apply(lambda row: row['elo_opponent'] + MANUAL_TEAM_ELO_ADJUSTMENT if row['opponent'] in team_list else row['elo_opponent'], axis=1)
        df['team_goals_scored_avg'] = df.apply(lambda row: row['team_goals_scored_avg'] * MANUAL_TEAM_GOAL_ADJUSTMENT if row['team'] in team_list else row['team_goals_scored_avg'], axis=1)
        df['opponent_goals_scored_avg'] = df.apply(lambda row: row['opponent_goals_scored_avg'] * MANUAL_TEAM_GOAL_ADJUSTMENT  if row['opponent'] in team_list else row['opponent_goals_scored_avg'], axis=1)
        df['team_attack_strength'] = df.apply(lambda row: row['team_attack_strength'] * MANUAL_TEAM_GOAL_ADJUSTMENT  if row['team'] in team_list else row['team_attack_strength'], axis=1)
        df['team_defense_strength'] = df.apply(lambda row: row['team_defense_strength'] * (1/MANUAL_TEAM_GOAL_ADJUSTMENT)  if row['team'] in team_list else row['team_defense_strength'], axis=1)
        df['opponent_attack_strength'] = df.apply(lambda row: row['opponent_attack_strength'] * MANUAL_TEAM_GOAL_ADJUSTMENT  if row['opponent'] in team_list else row['opponent_attack_strength'], axis=1)
        df['opponent_defense_strength'] = df.apply(lambda row: row['opponent_defense_strength'] * (1/MANUAL_TEAM_GOAL_ADJUSTMENT)  if row['opponent'] in team_list else row['opponent_defense_strength'], axis=1)